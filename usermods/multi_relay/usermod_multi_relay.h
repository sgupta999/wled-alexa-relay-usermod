...
class MultiRelay : public Usermod {
  ...
  private:
    ...
    // strings to reduce flash memory usage (used more than twice)
    const char* _name  = "MultiRelay";
    const char* _enabled  = "enabled";
    const char* _relay_str  = "relay";
    const char* _delay_str  = "delay-s";
    const char* _activeHigh = "active-high";
    const char* _external  = "external";
    const char* _alexaEnabled = "alexa-enabled";
    const char* _alexaName = "alexa-invocation-name";
    const char* _button  = "button";
    const char* _broadcast = "broadcast-sec";
    const char* _HAautodiscovery = "HA-autodiscovery";
  ...
  public:
     ...
     * switch relay on/off
     */
    void switchRelay(uint8_t relay, bool state) {
      if (relay>=MULTI_RELAY_MAX_RELAYS || _relay[relay].pin<0) return;
      pinMode(_relay[relay].pin, OUTPUT);
      digitalWrite(_relay[relay].pin, state ? !_relay[relay].mode : _relay[relay].mode); 
      _relay[relay].state = state;
      publishMqtt(relay);
      //publishAlexa(relay);
    }
    ...
    void addToJsonInfo(JsonObject &root) {
      if (enabled) {
        ...
        for (int i=0; i<MULTI_RELAY_MAX_RELAYS; i++) {
          if (_relay[i].pin<0) continue;
          uiDomString = F("<small><bold>");
          uiDomString += (_relay[i].alexa) ? _relay[i].alexaName : F("Relay ");          
          if (!_relay[i].alexa) uiDomString += i ;
          uiDomString += F("</bold><sub>");
          uiDomString += (_relay[i].external) ? F("ext") : F("wled");
          uiDomString += (_relay[i].alexa) ? F(", alexa") : F("") ; 
          uiDomString += "</sub></small>";
    ...
    void addToConfig(JsonObject &root) {
      ...
      for (int i=0; i<MULTI_RELAY_MAX_RELAYS; i++) {
        ...
        relay[FPSTR(_alexaEnabled)]   = _relay[i].alexa;
        relay[FPSTR(_alexaName)]   = _relay[i].alexaName;
    ...
    bool readFromConfig(JsonObject &root) {
      ...
      for (int i=0; i<MULTI_RELAY_MAX_RELAYS; i++) {
        ...
        _relay[i].alexa    = top[parName][FPSTR(_alexaEnabled)]   | _relay[i].alexa;
        _relay[i].button   = top[parName][FPSTR(_button)]     | _relay[i].button;
        _relay[i].alexaName = top[parName][FPSTR(_alexaName)]     | _relay[i].alexaName;
