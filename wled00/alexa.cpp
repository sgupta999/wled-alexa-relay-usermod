...
bool userModEnabled = false;
MultiRelay* multiRelay;  
Relay* alexaRelays;
std::map<int, int> alexaToRelayId;
...
void alexaInit()
{
  ...
  userModEnabled = true;
  multiRelay = (MultiRelay*)usermods.lookup(USERMOD_ID_MULTI_RELAY);
  #endif
  ...
  // the secod configured device for on/off or macros (added first, i.e. index 0)
 if (userModEnabled){
    alexaRelays = multiRelay->getRelays();
    for (int i=0; i< MULTI_RELAY_MAX_RELAYS; i++ ){
      if (alexaRelays[i].alexa){
        espalexaDevice = new EspalexaDevice(alexaRelays[i].alexaName, relaysOnAlexaChange, EspalexaDeviceType::extendedcolor);
        espalexa.addDevice(espalexaDevice);
        alexaToRelayId[espalexaDevice->getId()] = i;
      }
    }
  }
  ...
  if (!userModEnabled && alexaNumPresets) {
    String name = "";
...
void onAlexaChange(EspalexaDevice* dev){
  ...
  {    
    EA_DEBUGLN("setting alexa device on ");
    ...
    } else  // switch-on behavior for preset devices
    {
      ...
      {
        if (i == dev->getId() || userModEnabled) continue;
  ...
  {
    EA_DEBUGLN("setting alexa device off");
    if (!macroAlexaOff)
    {
      if (bri > 0)
      {
  ...
  {
    EA_DEBUGLN("setting brightness on alexa device");
    int inputBri = dev->getValue()-1;
    int presetIndex = (inputBri*100)/255 + (((inputBri*100) % 255) != 0);   
    DEBUG_PRINTLN(inputBri);    
    DEBUG_PRINT(F("Setting preset to: "));
    DEBUG_PRINTLN(presetIndex);
    if (presetIndex < ESPALEXA_MAXDEVICES){
      applyPreset(presetIndex, CALL_MODE_ALEXA);
    }else {
      bri = dev->getValue();    
    }    
    stateUpdated(CALL_MODE_ALEXA);
  ...
  {    
    EA_DEBUGLN("setting alexa device color");
...
{  
  ...
  if (m == EspalexaDeviceProperty::on)
  {
    multiRelay->switchRelay(alexaToRelayId.at(dev->getId()), true);
  }else if (m == EspalexaDeviceProperty::off) {
    multiRelay->switchRelay(alexaToRelayId.at(dev->getId()), false);
  }
}
